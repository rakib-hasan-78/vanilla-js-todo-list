    ...
    <header class="w-full">
      <main class="w-11/12 mx-auto">
        <div class="title-box p-2">
          <h1 id="title" class="text-7xl text-sky-50 text-center font-extrabold font-sans">
            <span class="item-one">Welcome</span>
            <span class="item-four">To</span>
            <span class="item-two">TODO</span>
            <span class="item-three">App</span>
          </h1>
        </div>
      </main>
    </header>
    ...
    <section id="app-required" class=" flex flex-1 items-center justify-center mb-14">
      <div class="container flex items-center justify-center">
          <div class="bg-gradient-to-t from-zinc-300 to-fuchsia-200 w-4/6 grid grid-cols-2 gap-3 rounded-lg shadow-2xl shadow-zinc-400 relative h-80">
            <div>
              <div class="text-contents flex flex-col space-y-3 p-2 mt-5">
                <h2 class="uppercase text-2xl text-sky-500 font-black font-sans">enjoy our product</h2>
                <p class="text-xl font-normal text-zinc-500">Experience exceptional quality, prompt delivery, and outstanding support with us.</p>
              </div>
            </div>
            <div class="flex items-center justify-center">
              <div class="w-3/4 bg-sky-50 rounded-md shadow-md shadow-stone-300 h-4/5">
                  <form action="#" class="w-full p-2 mt-5">
                    <div class="form-contents flex flex-col space-y-3">
                      ...
                      <div class="pt-2 input-container relative">
                        ...
                        <div class="error-manage w-full text-xs capitalize font-black mt-2 italic text-red-700 opacity-80 absolute left-0 -bottom-7 origin-bottom flex items-end justify-end"></div>
                      </div>
                      <!---button container--->
                      <div class="container">
                        <div class="flex flex-1 items-center justify-center mt-5">
                          <button type="submit" class="w-2/3 bg-cyan-600 py-3 px-5 text-purple-50 lowercase text-center font-serif ease-in-out transition-all hover: hover:bg-cyan-500 duration-300 rounded-md">click here</button>
                        </div>
...
textWaving('.item-one', '.item-two', '.item-three','.item-four')
  ...
  [...containers].map(container=>{
    ...
    form.addEventListener('click',()=>{
        [...containers].map(container=>{
            ...
            const line = container.querySelector('.elastic-line')
            const inputField = container.querySelector('.input');
            const errMsg = container.querySelector('.error-manage');
            
            // restoring input animation 
            if (document.activeElement !== inputField) {
                if (!inputField.value) {
                    animationRestored(placeholder,0,1,'<75%')
                }
            }
            inputField.addEventListener('input', (e)=>{
                 try {
                    let validator;
                    // check whether given is valid for name 
                    if (e.target.name==='name') {
                        validator = namePattern(e.target.value)
                    }
                    // check if company name is valid
                     else if (e.target.name==='company-name') {
                        validator = companyPattern(e.target.value)
                    }
                    // resetting both company and name validation in validator 
                    const {isValid , error} = validator;
                    if (isValid) {
                        colorize(line, placeholder, 'rgb(0, 180, 180)');
                        errMsg.textContent = ``;
                    } 
                    // if empty forms contains----->
